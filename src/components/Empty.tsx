...
export function Empty({ text, action }: { text: string; action?: string }) {
  ...
  return (
    <div className="animar-entrada mt-6 rounded-2xl border border-line bg-card px-6 py-12 text-center">
      {carregando && (
        <div className="mx-auto mb-3 flex h-2 w-2 items-center justify-center">
          <span className="h-2 w-2 rounded-full bg-brand animar-pulso" />
        </div>
      )}
